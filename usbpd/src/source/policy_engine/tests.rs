...
use super::Source;
use crate::dummy::{get_dummy_source_capabilities, get_source_capability_request};
...
use crate::{counters::{Counter, CounterType}, dummy::{DummyDriver, DummySourceDevice, DummyTimer, MAX_DATA_MESSAGE_SIZE}, protocol_layer::message::header::{ControlMessageType, Header}};
...
fn request_capability_message(message_id: u8, highest_power: bool) -> Message {
    let source_capabilities = SourceCapabilities(heapless::Vec::from_slice(get_dummy_source_capabilities().as_slice()).unwrap());
    ...
    let data = match highest_power {
        true => Data::Request(PowerSource::new_fixed(
                    CurrentRequest::Highest,
                    VoltageRequest::Highest,
                    &source_capabilities
                ).unwrap()),
        false => Data::Request(PowerSource::new_fixed(
                    CurrentRequest::Highest,
...
async fn test_negotiation() {
    ...
    policy_engine.run_step().await.unwrap(); {
        ...
        let sent_capabilities = Message::from_bytes(&policy_engine.protocol_layer.driver().probe_transmitted_data()).unwrap();
        assert_eq!(
    ...
    policy_engine.run_step().await.unwrap();
    eprintln!("- Ran Step 4: {0:?}", policy_engine.state);
    assert!(matches!(policy_engine.state, State::CapabilityResponse(CapabilityResponse::Reject)));
...
async fn test_discovery() {
    ...
    loop {
        if !matches!(policy_engine.state, State::Discovery) { break; }

        // `Discovery` -> `SendCapabilities`
        policy_engine.run_step().await.unwrap();
        discovery_iterations += 1;

        if discovery_iterations > MAX_DISCOVERY_ITERS { break; }
        if !matches!(policy_engine.state, State::SendCapabilities) { break; }

        // `SendCapabilities` -> `Discovery`
        policy_engine.run_step().await.unwrap();
    }
    eprintln!("- Ran Step 3: {0:?}, discovery state iterations = {1:?}", policy_engine.state, discovery_iterations);
